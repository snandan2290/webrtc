...
export class Registerer {
    ...
    constructor(userAgent, options = {}) {
        this.env_type = userAgent.extraHeaders[3];
        if(this.env_type && this.env_type == 'Env-Type: primary'){
            this.primaryoptionsdata = options.requestDelegate;
        }else if(this.env_type && this.env_type == 'Env-Type: secondary'){
            this.secoptionsdata = options.requestDelegate;
        }
        this.disposed = false;
     ...
     */
    register(options = {}) {
        //Since registration check happening without requestDelegate data
        //event is not triggering in the application so handling requestData to 
        //capture register events on register success and failure
        console.log('register options value::', options);
        if(options && options.requestOptions){
            this.env_type = options.requestOptions.extraHeaders[3];
            console.log('env type', this.env_type);
          if(this.env_type && this.env_type == 'Env-Type: primary' && this.primaryoptionsdata == undefined){
             this.primaryoptionsdata = options;
          }else if(this.env_type && this.env_type == 'Env-Type: secondary' && this.secoptionsdata == undefined){
            this.secoptionsdata = options;
          }        
        }else {
            options = this.primaryoptionsdata ? this.primaryoptionsdata : this.secoptionsdata; 
        }
        console.log('primary options  value final::', this.primaryoptionsdata);
        console.log('secondary options  value final::', this.secoptionsdata);
        ...
        const outgoingRegisterRequest = this.userAgent.userAgentCore.register(this.request, {
            ...
            onReject: (response) => {
                ...
                this._retryAfter = undefined;
                this.waitingToggle(false);
                this.logger.log("Storing Waiting value to session");
                sessionStorage.setItem('waiting_value_on_fail_register', 'false');
                sessionStorage.setItem('register_fail_status_code', response.message.statusCode);
            },
            onTrying: (response) => {
     ...
     */
    registered(expires) {
	expires = expires/2 + 60;
        this.logger.log("New expire value = " + expires);
        this.clearTimers();
    ...
    stateTransition(newState) {
        ...
        this.logger.log("Storing Registration value to session");    
        sessionStorage.setItem('registration_value', this._state);
